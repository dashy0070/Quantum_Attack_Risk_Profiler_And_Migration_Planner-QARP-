...
root_dir = r"D:\Anirban\000000_Mtech Reva\CapStone_2"

# 25 Verified IEEE References
verified_references = [
...
for dirpath, _, filenames in os.walk(root_dir):
    for f in filenames:
        if f.endswith('.docx') and not f.startswith('~$') and 'pre_citations_backup' not in f:
            full_path = os.path.join(dirpath, f)
            try:
                doc = docx.Document(full_path)
                
                # Check if this document has the old 38-reference list or references section
                has_ibm = any('IBM Q Team' in p.text or 'attack simulations' in p.text for p in doc.paragraphs)
                has_bib = any(p.text.strip().lower() in ['bibliography', 'references'] for p in doc.paragraphs)
                
                if has_ibm or (has_bib and 'Capstone' in f):
                    print(f"\nProcessing matching document: {full_path}")
                    
                    # 1. Update in-text citations
                    rep_count = 0
                    for p in doc.paragraphs:
                        for target, rep in in_text_replacements:
                            if target in p.text:
                                p.text = p.text.replace(target, rep)
                                rep_count += 1
                    print(f"  -> Applied {rep_count} in-text replacements.")
                    
                    # 2. Replace old bibliography with verified 25 references
                    bib_idx = None
                    app_idx = None
                    for i, p in enumerate(doc.paragraphs):
                        txt = p.text.strip().lower()
                        if txt in ['bibliography', 'references']:
                            bib_idx = i
                        elif bib_idx is not None and (txt.startswith('appendix') or txt.startswith('table:')):
                            app_idx = i
                            break
                    
                    if bib_idx is not None:
                        end_idx = app_idx if app_idx is not None else len(doc.paragraphs)
                        
                        # Collect indices of old reference paragraphs starting with [1], [2], etc.
                        old_ref_indices = []
                        for i in range(bib_idx + 1, end_idx):
                            if re.match(r'^\s*\[\d+\]', doc.paragraphs[i].text):
                                old_ref_indices.append(i)
                        
                        if old_ref_indices:
                            print(f"  -> Found {len(old_ref_indices)} old references from P{old_ref_indices[0]} to P{old_ref_indices[-1]}. Replacing them...")
                            # Overwrite existing paragraphs with new references
                            for idx, new_ref in enumerate(verified_references):
                                if idx < len(old_ref_indices):
                                    doc.paragraphs[old_ref_indices[idx]].text = new_ref
                                    if doc.paragraphs[old_ref_indices[idx]].runs:
                                        doc.paragraphs[old_ref_indices[idx]].runs[0].font.name = 'Times New Roman'
                                else:
                                    # Insert extra after the last one
                                    p_ins = doc.paragraphs[old_ref_indices[-1]].insert_paragraph_before(new_ref, style='Normal')
                                    if p_ins.runs: p_ins.runs[0].font.name = 'Times New Roman'
                            
                            # If there were more old references (e.g. 38) than new (25), clear the leftover ones
                            if len(old_ref_indices) > len(verified_references):
                                for k in range(len(verified_references), len(old_ref_indices)):
                                    doc.paragraphs[old_ref_indices[k]].text = ""
                        else:
                            # Insert before Appendix
                            target_p = doc.paragraphs[app_idx] if app_idx is not None else doc.paragraphs[-1]
                            for new_ref in verified_references:
                                p_ins = target_p.insert_paragraph_before(new_ref, style='Normal')
                                if p_ins.runs: p_ins.runs[0].font.name = 'Times New Roman'
                            print("  -> Inserted all 25 references under Bibliography.")

                    # Save updated document
                    doc.save(full_path)
                    print(f"  -> Successfully updated and saved: {full_path}")
                    files_updated += 1
            except Exception as e:
                print(f"Could not process {full_path}: {e}")

# Also create a standalone verified copy in Final Viva and Demo
final_copy = os.path.join(root_dir, "Final Viva and Demo", "FINAL_QARP_REPORT_VERIFIED_BIBLIOGRAPHY.docx")
src_file = os.path.join(root_dir, "Final Viva and Demo", "2_CS_13_Capstone Project_Quantum_Attack_Risk_Profiler_And_Migration_Planner(QARP)_2026_v6 1.docx")
if os.path.exists(src_file):
    import shutil
    shutil.copy2(src_file, final_copy)
    print(f"\n[DONE] Also created fresh dedicated copy:\n  {final_copy}")

print(f"\nFinished! Total documents updated: {files_updated}")
